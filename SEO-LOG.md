...
## 2026-09-14 — Homepage H1 superlative removed; Murrumbeena carport added

Two decisions from Jimmy, both acted on.

### The homepage H1 no longer claims a ranking

**Was:** `Melbourne's Best-Rated Local House Painters`
**Now:** `Melbourne's 5-Star Rated Local House Painters`

"Best-rated" asserts a ranking against every painter in Melbourne and nothing supports
it. **5.0 from 17 Google reviews supports "5-star rated"** — and that exact phrasing is
already used in `llms.txt` ("a 5-star rated house painting company with a verified 5.0
Google rating"), so the two surfaces now agree instead of one overclaiming.

Raised on 2026-09-13 by the external audit, held for three messages because this is the
homepage's **only keyword-bearing heading** and rewording it is an SEO decision, not a
copy one. Jimmy authorised it on 2026-09-14 ("Yes fix the title").

Changed in **three places, all of which had to move together**:

- `client/src/pages/Home.tsx` — the React H1
- `client/index.html` — the pre-rendered homepage H1 (the homepage is NOT generated by
  `generate-static-pages.mjs`; there is no `public/index.html`, so this file is edited by
  hand and will silently disagree with the React layer if it is missed)
- `client/src/components/Hero.tsx` — a comment quoting the H1 to explain why the hero
  slogan is an `h2`

⚠️ **`<title>` and `og:title` were already clean** — `Jetblack Painting | House &
Commercial Painters Melbourne` — and were **not touched**. A settled title is worth more
than a tidier one, and the superlative was never in it.

⚠️ Three `Melbourne's best-…` matches were checked and **deliberately left**:
`NorthcotePainters` ("one of Melbourne's best-known café and lifestyle strips") and
`ElsternwickPainters` ("some of Melbourne's best-preserved Art Deco apartment
buildings"). Those describe the suburbs, not Jetblack. Not superlative claims about the
business.

⚠️ Still outstanding, not actioned: one **GBP service description** (interior painting)
ends "Elevate your space with Melbourne's best." Same class of claim, on the live Google
listing. It needs a `google_my_business` write, so it waits for Jimmy to name the change.

### Murrumbeena: second frame added, carport confirmed in scope

Jimmy confirmed the carport was part of the Murrumbeena job, so the photo he sent on
2026-09-14 earns a place — **but not as a second project.** It is the same house, same
day, same job as `project-murrumbeena-tudor-exterior.webp`: identical ladder position,
identical stack of paint tins, identical water bottle on the same brick pier. Publishing
the full frame would have put two near-identical wide shots on one page.

Instead it is **cropped to the carport gable** (`RECT=60,470,980,590`) — the battens,
barge board, fascia, gutter, downpipe and the junction where charcoal trim meets
untouched red brick. That is a detail the published wide shot does not show.

⚠️ **First crop attempt was wrong and was redone.** `RECT=0,450,1150,820` pulled in the
red car, the ladder and the stacked tins across the foreground; it read as a work site
rather than a finished detail. Rendered it beside the existing image before accepting it
— do that for every crop.

Colours are named (Dulux Monument on the trim, Dulux Silkwort on the render) **only
because this is the same job as the image above**, whose colours Jimmy confirmed on
2026-09-07. Do not extend colour naming to any other job without that confirmation.

### ⚠️ Branch hazard hit and avoided — read this before any reset

PR #289 was open and unmerged, and its commit existed **only on the remote branch**.
Work for this entry was started from a branch reset to `origin/main`, which did not
contain it. A push at that point would have force-overwritten #289 and destroyed it.

Caught by checking `git merge-base --is-ancestor origin/<branch> origin/main` before
pushing. Recovered by resetting to the **remote branch tip** rather than main, restoring
the four hand-edited `client/` files, and re-running the generators so the output carries
both changesets. Verified afterwards that #289's work survived (sameAs 111/127, 10
llms.txt service links, captionless alts present) alongside the new work.

**Rule: before pushing to the designated branch, check whether the remote tip is an
ancestor of main. If it is not, it carries unmerged work and must be the base.**

### Checks

- All three layers regenerated; second pass **stable**, no drift
- Near-duplicate 30.9% avg / 53.7% worst, 0 over gate; metadata clean
- 0 JSON-LD parse errors; `aggregateRating` still 0 in static pages
- `llms.txt` root and `public/` byte-identical
...
## 2026-09-14 — Google Ads history recovered: it ran once, and it was cheap

Jimmy asked what to do about the suspended Ads account. Windsor's `google_ads` connector
turned out to hold history nobody had seen before — earlier entries recorded the account as
"not connected to Windsor", so this is the first look at it.

### The only campaign ever run

**"Want A Professional Painter?" — 18 November to 1 December 2025, 14 days.**
...
⚠️ **The zero does NOT mean it produced no work.** GA4 has never recorded a single row, and
`generate_lead` / `phone_call_click` were only deployed in September 2026 — nine months
*after* this campaign ran. Nothing was measuring, so the correct reading is **unmeasured**,
not "failed". Do not cite this as evidence that ads do not work for this business.

What the numbers do support: **9.2% CTR and $1.58 a click are cheap** for a trade in
Melbourne. 95 clicks in a fortnight is more site traffic than organic produces in a year.

### Context — what the other channels actually deliver

| Channel | Window | Result |
| --- | --- | --- |
| Organic search | 28 days | 14,019 impressions → **6 clicks** (4 of them brand) |
| Google Business Profile | 90 days | **3 phone calls**, 25 website clicks |
| Google Ads | 14 days, Nov 2025 | 95 clicks at $1.58 |

Non-brand commercial clicks from organic run at roughly **one a month**. The off-page
authority ceiling means that does not change quickly. Paid is the only fast lever available.

### What cannot be done from here

**The suspension cannot be fixed through this tooling.** Google Ads UI is blocked from the
sandbox, and there is no appeal endpoint. Windsor's `google_ads` write actions are campaign
management — `create_campaign`, `push_keywords`, `set_campaign_budget`, bidding — and all of
them are meaningless against a suspended account. **Do not attempt them.** Only Jimmy can
read the suspension notice and appeal.

### The sequencing that matters

1. **GA4 first.** Property 545100608 has recorded nothing for two years. Relaunching ads
   before it works repeats the November 2025 mistake exactly: money spent, outcome unknown.
2. **Then the appeal**, once Jimmy has read the reason in the account.
3. **Then relaunch**, with `generate_lead` and `phone_call_click` marked as key events and
