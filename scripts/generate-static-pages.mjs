...
<head>
  ...
  <script>
    /* The stub is installed SYNCHRONOUSLY and only the network fetch is
       deferred. It used to sit inside _loadGA, which meant window.gtag did
       not exist until requestIdleCallback ran, so lib/analytics.ts fell back
       to pushing ["event", name, params] onto dataLayer by hand. That is NOT
       the shape gtag queues in — gtag pushes its own arguments object — so an
       early tap on the sticky call bar was being queued in a form gtag.js is
       not documented to replay. The phone tap is the conversion that matters
       most for a painter, so it must not depend on that.
       Defining the stub costs no request: gtag() only pushes to an array
       until the library arrives, so LCP is unaffected and the fetch stays on
       idle exactly as before. This is Google's own documented snippet order.
       ⚠️ Mirrored between client/index.html and generate-static-pages.mjs.
       Keep both in step — GA4 was in only one of them until 2026-08-30. */
...
function suburbServiceCards({ suburb, localExpertise, propertyTypes }) {
  ...
  let interior;
  if (has.heritage) {
    interior = `Walls, ceilings, trims and doors in ${suburb}, including the ornate cornices, ceiling roses and deep skirtings period homes here tend to have. Detailed cutting-in, surfaces patched and sanded, premium low-sheen and enamel finishes.`;
  } else if (has.strata) {
...
const allSuburbPages = [
  ...
  { route: "/painter-lyndhurst", source: "LyndhurstPainters.tsx" },
  ...
  { route: "/painter-rowville", source: "RowvillePainters.tsx" },
  ...
  { route: "/painter-wantirna-south", source: "WantirnaSouthPainters.tsx" },
...
const servicePages = [
  {
    ...
    extraSections: [
      ...
      },
      {
        ...
        paragraphs: [
          "Room count and size are the obvious factors, but wall and ceiling condition usually matters more. A recently painted, sound surface is quick to prepare. A home with cracked cornices, water stains, old wallpaper or years of nail holes needs far more work before a topcoat goes anywhere near it. That preparation is the difference between a repaint that looks factory-fresh and one that shows every flaw underneath.",
          "Colour change matters more than most homeowners expect. Going from a dark feature wall to a light colour often needs an extra coat or a tinted primer for even coverage. So does covering a strong existing colour. Either way that is more product and more labour than repainting a similar colour over itself.",
          "Ceiling height and access change the job too: standard 2.4m ceilings are straightforward, while raked ceilings, stairwells and double-height living areas need scaffolding or specialised access equipment. Paint quality and finish level are the fourth factor. A washable low-sheen in a high-traffic hallway costs more to supply than a basic flat finish. Two-pack enamel on kitchen cabinetry or trims costs more again. Both last considerably longer.",
  ...
  },
  {
    ...
    extraSections: [
      ...
      },
      {
        ...
        paragraphs: [
          "Substrate condition is usually the biggest factor. A weatherboard home with sound, well-adhered paint is quick to prepare. One with flaking coatings, damaged boards and years of moisture damage is not. That needs real repair and priming before a single topcoat goes on. Rendered and brick homes are generally more stable but can need crack repair, chalk removal, or a sealer if they've never been painted before.",
          "Height and access are the second driver. A single-storey home with clear ground access is straightforward. A two-storey house, steep gables or a home on a slope needs scaffolding or elevated access equipment. That adds real cost and time.",
          "Colour change and coat count matter too. Going from a dark colour to a light one, or covering chalky, faded paint, sometimes needs an extra coat for even coverage. Coastal and heavily sun-exposed properties also benefit from a higher-grade weather-resistant system, which costs more to supply but holds up considerably longer against Melbourne's UV and salt exposure.",
    ...
    extraSections: [
      {
        ...
        paragraphs: [
          "Concrete and terracotta tile is porous. As the original surface treatment wears off, the tile itself starts absorbing water. That is why an unsealed roof gets heavier, dries slower after rain, and grows moss faster every year. Restoring a tile roof means dealing with that porosity directly. A penetrating sealer goes on before the colour coats. The topcoat then sits on a stable surface, instead of soaking straight into bare tile.",
          ...
          "Cement sheet is a third case again. This is the fibre-cement roofing common on older extensions and outbuildings. Like tile, it chalks and becomes porous with age. Unlike tile, it is prone to hairline cracking, so it needs a flexible crack-bridging primer. A rigid one telegraphs every existing crack straight through the new coating.",
        ],
      },
      {
        heading: "What roof cleaning and repair actually involves",
        paragraphs: [
          "Every job starts with high-pressure cleaning, and it matters more than people expect. Moss, lichen and years of grime sit between the coating and the roof surface. A coating applied over any of that peels away within a season, taking the moss and the paint with it. Cleaning also does the job of revealing what's actually wrong with the roof, which brush-applying paint over a dirty surface would otherwise hide.",
          "On tile roofs, cleaning usually turns up cracked or slipped tiles, and ridge capping where the mortar bedding has failed. Both get fixed before priming starts. A coating is not a structural repair and will not stop a slipped tile leaking. On metal roofs, cleaning is followed by a fixing-by-fixing rust check. Every screw, cut edge and valley is inspected on its own, rather than assessed as one surface. That is genuinely where a metal roof fails first.",
          "This is also the stage where we make the call on whether painting is the right job at all. Extensively cracked tiles, failed capping across large sections, or sheeting rusted through rather than surface-affected are repair problems, not paint problems. Coating over them seals the fault in where nobody can see it. That is worse than leaving it exposed. We say so at the quote stage, before any work is booked.",
        ],
      },
      {
        ...
        paragraphs: [
          ...
          "What determines how long a membrane system lasts is the applied thickness, not the number of coats. Two thin coats and three thin coats can both fall short of the specification a genuine 10 to 15 year result needs. We apply to the coating manufacturer's specified thickness, not a fixed number of passes. That is the difference between a roof coating still performing at year twelve and one chalking out by year six.",
          "Heat-reflective coatings work by reflecting more solar radiation than a standard dark roof colour absorbs, which measurably reduces how much heat transfers into the roof cavity below. Lighter colours and purpose-made reflective coatings are worth considering on any north or west-facing roof. They are especially worth it on a roof already due for restoration. The reflective benefit costs nothing extra in preparation, because the roof needs cleaning and coating either way.",
        ],
      },
    ],
    faqs: [
    ...
    extraSections: [
      {
        ...
        paragraphs: [
          ...
          "That means breaking the scope down by area rather than quoting the building as one lump figure — lobby, stairwells, façade, car park. A committee weighing priorities against a budget can then see where the cost actually sits. It can stage the work, or split it across financial years if that is how the money has to work. It also means the insurance documentation is ready before anyone asks for it, rather than chased down after the meeting.",
        ],
      },
      {
        ...
        paragraphs: [
          ...
          "For complexes with more than one building, the same logic extends across the site. The works can run as one continuous staged program. Or they can be split building by building against separate budget approvals — whichever suits how the committee wants to fund it. Either way the schedule goes out early enough for the committee to circulate it to owners and tenants before scaffolding arrives, not after.",
        ],
      },
      {
        ...
        paragraphs: [
          ...
          "That assessment happens before the price is finalised. Access equipment is one of the biggest cost variables in a multi-storey repaint. Get it wrong and you either blow the budget, or turn up on site to find the booked equipment cannot reach the work. We also account for whatever the owners corporation's bylaws say about common property access, so the equipment plan does not clash with the strata rules at the last minute.",
        ],
      },
    ],
    faqs: [
    ...
    extraSections: [
      {
        ...
        paragraphs: [
          ...
          "Mechanical preparation solves it by removing that layer entirely, along with old paint, sealers, glue residue and surface staining, and leaving a profile the first coat can key into. This is also the stage that exposes what the floor is actually like underneath. Cracks hidden under dirt, patches of previous repair, soft or crumbling areas, old oil soaked into the slab — all of it surfaces at this stage. Every one of them needs dealing with before a coating goes anywhere near the floor.",
      ...
      },
      {
        ...
        paragraphs: [
          ...
          "Solid colour coatings suit spaces where the floor should read as one clean surface rather than a decorative feature. Think plant rooms, storage areas and back-of-house commercial spaces. They also suit line-marked areas, where the marking needs to stand out against a plain background. They're also the easier finish to touch in locally later, which matters in a working space that takes knocks.",
          "Clear-sealed concrete is the lightest-touch option: the slab is ground, any repairs made good, and a clear sealer applied so the concrete itself stays visible. It suits polished-look interiors, and spaces where the raw concrete is genuinely part of the design. It is also the option that shows the slab's own history most honestly. Depending on the floor, that is either exactly what you want or exactly what you don't.",
        ],
      },
      {
        heading: "Garages, Warehouses and Commercial Floors",
        paragraphs: [
          "A domestic garage is usually the most straightforward version of this work. It is also where the visual difference is starkest. A stained, dusty slab becomes a floor that reads as a finished room rather than a leftover space. Concrete dust is the underrated part: an uncoated slab sheds fine dust continuously onto everything stored on it, and sealing the surface stops that at the source.",
          "Warehouse and workshop floors are a different problem, and it's rarely the coating itself. It's staging. The floor has to come out of service in sections, so the business keeps operating. Stock and racking have to be moved and moved back. Each section needs its cure time protected from foot and vehicle traffic before the next one starts. That planning is the part worth getting right at the quote, because a schedule that ignores it costs the business more than the floor does.",
